#!/usr/bin/env python3
"""
SquashPlot Deployment Readiness Checker - OS Compatible Version
Comprehensive pre-deployment validation for all operating systems
"""
...
class DeploymentReadinessChecker:
    """OS-compatible deployment readiness validation"""
    ...
    def run_readiness_check(self) -> Dict:
        ...
        self.safe_print("SquashPlot Deployment Readiness Checker - OS Compatible")
        self.safe_print("=" * 70)
        self.safe_print("Validating system readiness for production deployment...")
        self.safe_print("")
        
        # Run all readiness checks
        for category, config in self.readiness_checks.items():
            self.safe_print(f"Checking {category.upper()} readiness...")
    ...
    def _check_category(self, category: str, config: Dict):
        ...
        status_icon = "[OK]" if success_rate == 100 else "[WARNING]" if success_rate >= 70 else "[ERROR]"
        self.safe_print(f"   {status_icon} {category.title()}: {passed_checks}/{total_checks} checks passed ({success_rate:.1f}%)")
        ...
        if check == "hardened_bridge_exists":
            ...
            if os.path.exists("secure_bridge_hardened.py"):
                try:
                    ...
                    return {
                        "status": "BLOCKING",
                        "issue": "Cannot read hardened bridge app",
                        "recommendation": "Fix hardened bridge app file"
                    }
            else:
                return {
                    "status": "BLOCKING",
        ...
        if check in legal_files:
            ...
            if os.path.exists(filename):
                try:
                    ...
                    return {
                        "status": "WARNING",
                        "issue": f"{filename} exists but cannot be read",
                        "recommendation": f"Fix {filename} file encoding"
                    }
            else:
                return {
                    "status": "BLOCKING",
        ...
        if check == "gdpr_compliance":
            if os.path.exists("PRIVACY_POLICY.md"):
                try:
                    ...
                    return {
                        "status": "WARNING",
                        "issue": "Cannot read privacy policy",
            ...
            if os.path.exists("ACCESSIBILITY_STATEMENT.md"):
                try:
                    ...
                    return {
                        "status": "WARNING",
                        "issue": "Cannot read accessibility statement",
    ...
    def _generate_readiness_report(self) -> Dict:
        ...
        self.safe_print("\n" + "=" * 70)
        self.safe_print("DEPLOYMENT READINESS REPORT")
        self.safe_print("=" * 70)
        ...
        if self.blocking_issues:
            ...
            status_icon = "[NOT READY]"
        
        self.safe_print(f"{status_icon} Deployment Status: {deployment_status}")
        self.safe_print(f"Overall Readiness: {overall_readiness:.1f}%")
        self.safe_print(f"Total Checks: {total_checks}")
        self.safe_print(f"Passed Checks: {total_passed}")
        self.safe_print(f"Blocking Issues: {len(self.blocking_issues)}")
        self.safe_print(f"Warnings: {len(self.warnings)}")
        
        # Print blocking issues
        if self.blocking_issues:
            self.safe_print(f"\nBLOCKING ISSUES (Must be resolved before deployment):")
            for i, issue in enumerate(self.blocking_issues, 1):
                self.safe_print(f"   {i}. [{issue['category'].upper()}] {issue['issue']}")
                self.safe_print(f"      Recommendation: {issue['recommendation']}")
        
        # Print warnings
        if self.warnings:
            self.safe_print(f"\nWARNINGS (Should be addressed):")
            for i, warning in enumerate(self.warnings, 1):
                self.safe_print(f"   {i}. [{warning['category'].upper()}] {warning['issue']}")
                self.safe_print(f"      Recommendation: {warning['recommendation']}")
...
def main():
    ...
    print("SquashPlot Deployment Readiness Checker - OS Compatible")
    print("=" * 70)
    ...
    try:
        ...
        if report["deployment_status"] == "READY FOR DEPLOYMENT":
            ...
            print(f"\n[WARNING] System is not ready for deployment. Please address the issues above.")
        
    except Exception as e:
        print(f"[ERROR] Error running readiness check: {e}")
