...
class XBowF2CPUReconnaissanceSystem:
    ...
    async def perform_passive_reconnaissance(self, target: XBowTarget) - ReconnaissanceResult:
        ...
        try:
            ...
            whois_data  await self._perform_whois_reconnaissance(target.domain)
            data_gathered["whois"]  whois_data
    ...
    async def _perform_whois_reconnaissance(self, domain: str) - Dict[str, Any]:
    ...
    async def _perform_tech_stack_reconnaissance(self, domain: str) - Dict[str, Any]:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(f"https:{domain}", timeout10) as response:
                ...
                for service, url in service_urls.items():
                    try:
                        if service in ["http", "https"]:
                            async with session.get(url, timeout5) as response:
                                services[service]  {
